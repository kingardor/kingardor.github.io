...
const DEFAULT_GLOW_COLOR = '239, 43, 58';
...
const ParticleCard = ({
  ...
  return (
    <Tag ref={cardRef} className={`${className} particle-container`} style={{ ...style, position: 'relative', overflow: 'hidden' }} {...linkProps}>
      {children}
    </Tag>
  );
};
    ...
    <>
      ...
      <div className="card-grid bento-section" ref={gridRef} style={{ '--bento-cols': columns }}>
        {cards.map((card, index) => {
          const baseClassName = [
            'magic-bento-card',
            card.feature ? 'magic-bento-card--feature' : '',
          ...
          const linkProps = card.href
            ? { href: card.href, target: card.href.startsWith('#') ? undefined : '_blank', rel: card.href.startsWith('#') ? undefined : 'noreferrer' }
            : { href: '#', onClick: e => e.preventDefault() };

          const content = (
            <>
              <div className="magic-bento-card__header">
                <div className="magic-bento-card__code">{card.code}{card.feature && ' · FEATURED'}</div>
                {card.href && !card.href.startsWith('#') && (
                  <svg className="magic-bento-card__arrow" width="12" height="12" viewBox="0 0 12 12" fill="none">
                    <path d="M3 9L9 3M9 3H4M9 3V8" stroke="currentColor" strokeWidth="1.2" strokeLinecap="square"/>
                  </svg>
                )}
              </div>
              <div className="magic-bento-card__content">
                <h3 className="magic-bento-card__title">{card.name}</h3>
                <p className="magic-bento-card__description">{card.desc}</p>
              </div>
              <div className="magic-bento-card__tags">
                {(card.tags || []).map(t => <span className="magic-bento-card__tag" key={t}>{t}</span>)}
              </div>
            </>
          );
          ...
          if (enableStars) {
            return (
              <ParticleCard key={index} as="a" className={baseClassName} style={cardStyle} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism} {...linkProps}>
                {content}
              </ParticleCard>
            );
          }
          ...
          return (
            <a key={index} className={baseClassName} style={cardStyle} {...linkProps}>
              {content}
            </a>
          );
        })}
